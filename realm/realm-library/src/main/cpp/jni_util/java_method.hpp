...
public:
    ...
    //
    // NOTE: For anyone wants to implement this method, please DON'T. There might be a bug in JVM implementation that
    // the jmethodID retrieved from jobject's jclass would be invalid under some certain conditions.
    ...
    // For this constructor, no evidence shows that the jmethodID will be invalided if there is no ref to the jclass.
    // But just in case, we force the caller to keep a global ref by JavaClass to make sure we won't meet another jvm
    // bug.
